...
Das Programm "Prediction.py" wurde erweitert. Drei unterschiedliche Trainingsmodelle wurden in einem separaten Modul "CnnModels.py" in einer Klassenstruktur untergebracht. Eine Modellmethode der Klasse entspricht dem vorgegebenen Programmcode `self.mnist_cnn_modified()`. Der Coder der anderen beiden Methoden entstammt im Wesentlichen der Quelle "deep learning with python". Die Trainingsmethoden der Klasse CnnModels können aus dem auführbaren Pythonprogramm "Prediction.py" aufgerufen wenden. Getestet wurden: "inputLabel, predictionLabel= cnnPred.scnnMnist(imageIndex)" und "inputLabel, predictionLabel= cnnPred.mnist_cnn_modified(imageIndex)".
